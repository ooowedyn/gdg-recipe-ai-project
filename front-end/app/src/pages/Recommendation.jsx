...
function Recommendation() {
  ...
  // 주의: 여기서 추천을 자동 요청하지 않는다.
  // Filter 화면에서 넘어올 때(추천받기)와 아래 '재추천' 버튼에서만 요청한다(task 5).
  // 레시피 화면에서 뒤로 오면 이전 추천이 그대로 유지된다.
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 20px 6px', flexShrink: 0 }}>
        ...
        <button
          ...
          style={{
            ...
            color: '#FFF3DC',
            cursor: recsLoading ? 'default' : 'pointer',
            opacity: recsLoading ? 0.5 : 1
          }}
        >
      ...
      {recsLoading ? (
        <Loading
          ...
          messages={[
            '냉장고 재료를 살펴보는 중이에요…',
            '인터넷에서 실제 레시피를 검색하는 중이에요…',
            '초보자도 만들 수 있는 메뉴를 고르는 중이에요…',
            '조리 순서를 정리하는 중이에요…',
          ]}
          sub="실제 레시피를 웹에서 찾느라 20~40초 정도 걸릴 수 있어요."
        />
      ) : recommendedRecipes.length === 0 ? (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px', textAlign: 'center' }}>
          <p style={{ font: "600 14.5px 'Pretendard'", color: '#B7A595', lineHeight: 1.6 }}>
            아직 추천 메뉴가 없어요.<br />재료를 확인하고 조건에서 <b>메뉴 추천받기</b>를 눌러주세요.
          </p>
        </div>
      ) : (
        <div className="scrl" style={{ flex: 1, overflowY: 'auto', padding: '8px 22px 22px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <p style={{ font: "500 13.5px 'Pretendard'", color: '#9A8678', margin: 0 }}>가진 재료로 만들 수 있는 메뉴예요</p>
          ...
          {recommendedRecipes.map((r) => (
            ...
            >
              <div style={{ width: '96px', height: '96px', flexShrink: 0, background: '#F3EADB', border: '1.5px dashed #D8C7AC', borderRadius: '16px', display: 'flex', flexDirection: 'column', gap: '6px', alignItems: 'center', justifyContent: 'center', color: '#B7A595', font: "500 10.5px 'Pretendard'", overflow: 'hidden' }}>
                {r.image ? (
